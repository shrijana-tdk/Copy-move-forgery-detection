...
###############THRESHOLD_CONTROL########################
G2NN_THRESHOLD=0.8#0.5
MIN_CLUSTER_SIZE_CONTROL=4
MIN_NO_OF_MATCHES=8#16
MIN_DISTANCE=30 #clustering 30
G2NN_TO_SHOW=30 #30
########################################################
COLOR_IMAGE=[]
KP=[]
...
def calculateAccuracy(f1, f2, ground_truth):
    ...
    for kp1, kp2 in zip(f1, f2):
        x1, y1 = kp1.pt
        x2, y2 = kp2.pt
        true_positive = np.sum(np.logical_and(ground_truth[int(y1), int(x1)] == 255, ground_truth[int(y2), int(x2)] == 255))
        false_positive = np.sum(np.logical_and(ground_truth[int(y1), int(x1)] == 255, ground_truth[int(y2), int(x2)] == 0))
        true_negative = np.sum(np.logical_and(ground_truth[int(y1), int(x1)] == 0, ground_truth[int(y2), int(x2)] == 0))
        false_negative = np.sum(np.logical_and(ground_truth[int(y1), int(x1)] == 0, ground_truth[int(y2), int(x2)] == 255))
    '''
    ...
    '''        
    # Count the number of copy-move regions in the ground truth
    num_copy_move_regions = len(np.unique(ground_truth)) - 1
    #Calculate false negatives (missed copy-move regions)
    false_negative = num_copy_move_regions - true_positive
    #false_negative = len(ground_truth) - true_positive
    # Calculate true negatives
    true_negative = len(f1) - (true_positive + false_negative)
    '''
    ...
    recall = true_positive / (true_positive + false_negative)
        ...
        #accuracy = (true_positive + false_negative) / len(ground_truth)
...
def main(file_name):
    ...
    ground_truth = cv2.imread("im50_t_mask.png", cv2.IMREAD_GRAYSCALE)
    try:
